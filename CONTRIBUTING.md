...
When Task 7's development tooling is available, run the relevant `just` commands before submitting a change: `check`, `test`, `fmt`, `lint`, `rust-check`, `rust-test`, `proto-check`, and (with an Android SDK) `android-check`. Update architecture and glossary documentation when shared terminology or a boundary changes. By contributing, you agree to follow the [Code of Conduct](CODE_OF_CONDUCT.md).
